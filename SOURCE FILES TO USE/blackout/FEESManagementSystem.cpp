...
#include "FEESManagementSystem.h"
...
void GpioSet(GPIO_TypeDef* GPIO_PORT , uint16_t GPIO_PIN , int pin){
  ...
  HAL_GPIO_WritePin(GPIO_PORT, GPIO_PIN, 1);
	  ...
	  char DATA2[2];
	  itoa(pin, DATA2, 10);
	  char DATA3[]= " has been turned on! \r\n";

	  HAL_UART_Transmit(&huart3, &DATA1, sizeof(DATA1), 100);
	  HAL_UART_Transmit(&huart3, &DATA2, sizeof(DATA2), 100);
	  HAL_UART_Transmit(&huart3, &DATA3, sizeof(DATA3), 100);
...
void GpioReset(GPIO_TypeDef* GPIO_PORT , uint16_t GPIO_PIN , int pin){
  ...
  HAL_GPIO_WritePin(GPIO_PORT, GPIO_PIN, 0);
	  ...
	  char DATA2[2];
	  itoa(pin, DATA2, 10);
	  char DATA3[]= " has been turned off! \r\n";
